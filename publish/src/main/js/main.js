...
function initConfig() {
  ...
  $('.config-filename-preview').html(new Date('2022-01-01').format(config['config-filename'].value))
...
function addLisenter() {
  ...
  $('.config-filename').on('input propertychange', (e) => {
    ...
    if (rtn !== -1) {
      $('.config-filename-preview').html(`有非法字符【${e.target.value[rtn]}】`)
      return
    } else {
      $('.config-filename-preview').html(new Date('2022-01-01').format(e.target.value))
    }
  })
}
